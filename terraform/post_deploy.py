...
def post_deploy(
    ...
    if not os.path.exists(local_age):
        warn(f"Age key not found at {local_age}, skipping age-identity setup")
    else:
    ...
    ssh_exec(
        host_ip,
        "chmod 600 /etc/agenix/identities/age-key.txt /etc/ssh/age-identity",
        timeout=30,
    )

    # Step 3: Run NixOS activation to decrypt agenix secrets
    info(f"[{cloud}/{region}] Activating agenix secrets...")
    ssh_exec(
        host_ip,
...
def _connect_tailscale(host_ip: str) -> None:
    ...
    try:
        ...
        warn(f"Tailscale connect on {host_ip} had issues: {e}")
